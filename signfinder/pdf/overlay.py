...
# Целевая высота подписи в pt. Была 42 (15мм) — на реальном сравнении с
# кабинетом (v1.20.17) оказалась заметно крупнее, чем ручная простановка со
# стандартным (не изменённым) размером плейсера: тот при keep_proportion=True
# на широкой подписи садится по ширине бокса (130×54.6 CSS px), а не по
# высоте, и эффективно даёт меньший росчерк. 28pt — первая оценка по
# скриншоту сравнения, не точный расчёт (эффективный размер ручного
# плейсера зависит от ширины окна браузера и aspect ratio конкретной
# подписи пользователя — точного инварианта между ними нет).
DEFAULT_SIGNATURE_HEIGHT_PT = 28
    ...
    for m in matches:
        ...
        if use_signature and img_stream is not None:
            ...
            if getattr(m, "added_by", "") in ("manual_exact", "manual_click"):
                ...
                # v1.20.10: descender=0.18*sig_h (кап 8pt) — типичный визуальный
                # descender рукописного росчерка. При sig_h=42pt: descender=7.5pt,
                # подпись центром садится на линию подчёркивания. v1.20.9 (0.45,
                # кап 22pt) был overcorrection после фикса "подпись висит над
                # линией" — уехало в другую крайность (подпись под линией).
