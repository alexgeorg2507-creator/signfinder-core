...
def run_pipeline_auto_1(
    ...
    def _is_reverse_pattern(p: str) -> bool:
        ...
        редкая, и вариант с одним переносом строки (_\nРоль) — типичный
    ...
    norm_count = 0
    for p in patterns:
        # Reverse-паттерны (линия→роль) НЕ нормализуем — сохраняем [\s\S] как
        # есть, чтобы ловить футеры вида '___\nЗаказчик' (двухколоночный подвал).
        if _is_reverse_pattern(p):
            ...
            continue
        # Forward-паттерны (роль→линия) — умная нормализация с одним \n.
