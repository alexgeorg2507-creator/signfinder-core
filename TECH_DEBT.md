...
| TD-10 | **DOCX→PDF футер не 1:1 с Word (шрифт + верстка).** Fix-9 препроцессор (`_preprocess_docx_for_libreoffice`, signfinder-api/app/routers/me.py) довёл до почти идеала: цвет линии #622422 vs эталон #622423 (измерено пиксельно), автоподстановка Cambria→Caladea подтверждена (fonts-crosextra-caladea в Dockerfile, шрифт реально применяется). Позиция "Страница N" исправлена явным `<w:tabs><w:tab w:val="right" w:pos="{вычислено из pgSz/pgMar}"/></w:tabs>` вместо голого `<w:ptab>` (который LibreOffice не поддерживает — подтверждено измерением, не предположением). Несмотря на это — по фидбеку владельца после деплоя шрифт и разметка страницы визуально всё ещё расходятся с оригиналом Word. Решено не продолжать погоню за pixel-perfect: LibreOffice — другой рендер-движок, полное совпадение нереалистично без непропорциональных трудозатрат. Принято как ограничение платформы. | v1.20.10–v1.20.11 (Fix-8/Fix-9) | Low — принято, не блокирует use case (документ читаем, подпись на месте, юридическая суть не страдает) |
